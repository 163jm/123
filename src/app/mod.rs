...
use crate::{
    ...
    inbound::{
        dns::DnsInbound,
        hysteria2_server::Hysteria2Inbound,
        mixed::MixedInbound,
        shadowsocks_server::ShadowsocksInbound,
        trojan_server::TrojanInbound,
        tuic_server::TuicInbound,
        tun::TunInbound,
        vless_server::VlessInbound,
        vmess_server::VmessInbound,
        ...
        for ib_config in &config.inbounds {
            match ib_config {
                ...
                InboundConfig::Vless(c) => {
                    let tls_info = c.tls.as_ref().map(|t| {
                        if t.reality.is_some() { "reality" } else if t.enabled { "tls" } else { "none" }
                    }).unwrap_or("none");
                    let transport_info = match &c.transport {
                        None => "tcp",
                        Some(crate::config::inbound::ServerTransportConfig::Ws(_)) => "ws",
                        Some(crate::config::inbound::ServerTransportConfig::Xhttp(_)) => "xhttp",
                        Some(crate::config::inbound::ServerTransportConfig::Grpc(_)) => "grpc",
                        Some(crate::config::inbound::ServerTransportConfig::HttpUpgrade(_)) => "httpupgrade",
                        Some(crate::config::inbound::ServerTransportConfig::Tcp) => "tcp",
                    };
                    info!(
                        tag = %c.tag,
                        listen = %c.listen,
                        ...
                        "starting vless inbound"
                    );
                    let inbound = VlessInbound::new(c.clone(), tcp_tx.clone());
                    tasks.spawn(async move { inbound.run().await });
                }
                InboundConfig::Vmess(c) => {
                    let tls_info = c.tls.as_ref().map(|t| if t.enabled { "tls" } else { "none" }).unwrap_or("none");
                    info!(
                        tag = %c.tag,
                        listen = %c.listen,
                        port = %c.listen_port,
                        users = c.users.len(),
                        tls = tls_info,
                        "starting vmess inbound"
                    );
                    let inbound = VmessInbound::new(c.clone(), tcp_tx.clone());
                    tasks.spawn(async move { inbound.run().await });
                }
                InboundConfig::Trojan(c) => {
                    ...
                    info!(
                        ...
                        users = c.users.len(),
                        tls = tls_info,
                        "starting trojan inbound"
                    );
                    let inbound = TrojanInbound::new(c.clone(), tcp_tx.clone());
                    tasks.spawn(async move { inbound.run().await });
                }
                InboundConfig::Shadowsocks(c) => {
                    info!(
                        ...
                        port = %c.listen_port,
                        method = %c.method,
                        "starting shadowsocks inbound"
                    );
                    let inbound = ShadowsocksInbound::new(c.clone(), tcp_tx.clone());
                    tasks.spawn(async move { inbound.run().await });
                }
                ...
                InboundConfig::Tuic(c) => {
                    info!(
                        ...
                        port = %c.listen_port,
                        users = c.users.len(),
                        congestion = %c.congestion_control,
                        "starting tuic inbound"
                    );
