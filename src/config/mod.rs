        ...
        for ib in &self.inbounds {
            ...
            match ib {
                InboundConfig::Vless(c) => {
                    for user in &c.users {
                        validate_uuid(&user.uuid).with_context(|| {
                            format!("inbound vless '{tag}': user '{}' invalid UUID", user.name)
                        })?;
                    }
                }
                InboundConfig::Vmess(c) => {
                    for user in &c.users {
                        validate_uuid(&user.uuid).with_context(|| {
                            format!("inbound vmess '{tag}': user '{}' invalid UUID", user.name)
                        })?;
                    }
                }
                InboundConfig::Tuic(c) => {
                    for user in &c.users {
                        validate_uuid(&user.uuid).with_context(|| {
                            format!("inbound tuic '{tag}': user '{}' invalid UUID", user.name)
                        })?;
                    }
                }
                InboundConfig::Trojan(c) => {
                    anyhow::ensure!(!c.users.is_empty(), "inbound trojan '{tag}': no users configured");
                }
                InboundConfig::Shadowsocks(c) => {
                    let has_password = c.password.is_some() || !c.users.is_empty();
                    anyhow::ensure!(has_password, "inbound shadowsocks '{tag}': no password or users configured");
                }
                InboundConfig::Hysteria2(c) => {
                    anyhow::ensure!(!c.users.is_empty(), "inbound hysteria2 '{tag}': no users configured");
                    anyhow::ensure!(
                        c.tls.enabled,
                        "inbound hysteria2 '{tag}': TLS must be enabled (Hysteria2 requires QUIC+TLS)"
                    );
                }
                _ => {}
            }
